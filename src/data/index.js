...
// Build per-year question sets (45 per subject = 180 total, shuffled within each subject)
export function getQuestionsByYear(year) {
    const y = String(year);
    const shuffle = arr => [...arr].sort(() => Math.random() - 0.5);
    const physics = shuffle((physicsByYear[y] || []).slice(0, 45)).map((q, i) => ({
        ...q,
        index: i,
        subject: 'Physics',
        year,
    }));
    const chem = shuffle((chemByYear[y] || []).slice(0, 45)).map((q, i) => ({
        ...q,
        index: 45 + i,
        subject: 'Chemistry',
        year,
    }));
    const bio = shuffle((bioByYear[y] || []).slice(0, 90)).map((q, i) => ({
        ...q,
        index: 90 + i,
